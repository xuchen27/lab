...
using namespace std;
using namespace ros;
/***********NOTE: set these define will cause topic frequency drop to 50%***********/
//#define SENSOR_SHOW
//#define JUMP_SHOW
...
void decode_loop()
{
    ...
    case GYRO_X:
        ...
        if(dat_count == 0)
        {
            mpu9250.gyro_raw.gyro_xh = dat; raw_data[6] = dat;
            //printf("dat: %x-", dat);
            dat_count++;
        }
        else if(dat_count == 1)
        {
            mpu9250.gyro_raw.gyro_xl = dat; raw_data[7] = dat;
            //printf("%x ", dat);
    ...
    case GYRO_Y:
        ...
        if(dat_count == 0)
        {
            mpu9250.gyro_raw.gyro_yh = dat; raw_data[8] = dat;
            //printf("%x-", dat);
            dat_count++;
        }
        else if(dat_count == 1)
        {
            mpu9250.gyro_raw.gyro_yl = dat; raw_data[9] = dat;
            //printf("%x ", dat);
    ...
    case GYRO_Z:
        ...
        if(dat_count == 0)
        {
            mpu9250.gyro_raw.gyro_zh = dat; raw_data[10] = dat;
            //printf("%x-", dat);
            dat_count++;
        }
        else if(dat_count == 1)
        {
            mpu9250.gyro_raw.gyro_zl = dat; raw_data[11] = dat;
            //printf("%x\n", dat);
    ...
    case FRAME_END:
        ...
        if(dat_count == 2)
        {
            ...
            imu_msg.header.stamp    = ros::Time::now()+ros::Duration(Camera2ImuDelay);
            imu_pub.publish(imu_msg);
#ifdef FRAME_COUNTER
            ROS_INFO("Total frame receive: %ld ", frame_num);
